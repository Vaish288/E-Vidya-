...
export const Login = ({contract}) => {
  ...
  const loginfunc = async () => {
    ...
    try {
      let res;
      if (type === true) {
        res = await contract.methods.studentLogin(name, pass).call();
      } else {
        res = await contract.methods.educatorLogin(name, pass).call();
      }

      setLoading(false);

      if (res[0] !== -1 && res[0] !== 0) {
        setID({id: res[0], user: type ? "student" : "educator"});
        setFail(false);
        history.push(type ? "/student" : "/educator");
      } else {
        setFail(true);
      }
    } catch (error) {
      console.error("Error occurred during contract method call:", error);
      setLoading(false);
