...
import MetamaskError from "./metamask_error";
...
export const Educator = ({address, contract, t_contract, ts_contract}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (contract !== undefined && id.id != null) {
          ...
          setCourseID(parseInt(courseCount[0]) + 1);

          const educatorDetails = await contract.methods
            .getEducator(id.id)
            .call();
          if (educatorDetails !== null && educatorDetails !== undefined) {
            setAuthorID(educatorDetails[0]);
            setAuthor(educatorDetails[1]);
            if (educatorDetails[4]) setCount(educatorDetails[4].length);
          }
        }
        ...
        if (submissionsResponse.data !== undefined)
    ...
    getCourses();
    getBalance();
  }, [contract, id.id, getCourses, getBalance]);
